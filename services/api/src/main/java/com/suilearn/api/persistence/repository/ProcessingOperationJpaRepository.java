...
public interface ProcessingOperationJpaRepository extends JpaRepository<ProcessingOperationEntity, String> {
    ...
    @Query(value = """
        insert into processing_operations
          (id, operation_key, task_id, stage, state, attempt_count, adapter_version, created_at, updated_at, started_at)
        values
          (:id, :operationKey, :taskId, :stage, 'STARTED', 1, :adapterVersion, :now, :now, :now)
    ...
    @Query(value = """
        ...
        set state = 'STARTED', attempt_count = coalesce(attempt_count, 0) + 1, started_at = :now, updated_at = :now
