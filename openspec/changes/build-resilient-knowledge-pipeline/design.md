...
知识点详情的主要 CTA 为“生成面试题”。默认生成 1 道、中等难度、简答题；“更多设置”允许选择现有题型、难度和数量，单次最多 10 道。上限选择 10 是为了约束个人使用阶段的 AI 成本和逐题审核负担。生成难度到正式题库数值使用固定双向映射：`EASY → 2`、`MEDIUM → 3`、`HARD → 4`，保存和再次读取不得发生漂移。

生成请求只接受 `legacy=false`、`CONFIRMED`、`sourceOutdated=false` 且至少包含一条当前有效不可变引用的结构化知识点。旧版知识点必须先从现有 revision 重新生成结构化草稿并确认；来源已过期或引用无效时必须拒绝出题。请求携带已确认知识点的结构化内容及其证据引用，成功任务通过 task-scoped 批量结果地址返回一个或多个待审核草稿。

现有 AI 内容确认门禁保持不变：用户可编辑题干、选项、答案、解析、难度、题型和分类内容并保存或丢弃，但不得修改 `knowledgePointId`、`generationTaskId` 或 material/revision/citation 证据归属；保存后才进入正式题库。AI 返回的必填文本若 trim 后为空（包括仅空白或仅空白换行），任务必须失败且不得落草稿。知识点或题目生成失败不得改变资料 READY 或已确认知识点状态。
