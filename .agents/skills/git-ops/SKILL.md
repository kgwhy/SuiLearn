...
- 当前主开发分支是 `dev`；创建 Codex 分支时默认使用 `codex/` 前缀并从 `dev` 切出。发布顺序为 `commit -> pull --rebase -> push`。
- 任何提交都必须在暂存审阅后检查要提交代码是否暴露 key、secret 或 access token：优先运行 `python3 .agents/skills/git-ops/scripts/scan-staged-secrets.py`；无 Python 时退回 PowerShell 版本。预检命中或无法安全完成时，停止提交，不回显匹配内容，并请用户决定处置方式。
...
| 需要隔离并行任务或 worktree | [Worktree 隔离](references/worktree-setup.md) |
| 任务完成，准备合并或清理分支 | [完成分支](references/finish-branch.md) |

## 提交顺序

1. 检查状态与相关 diff，确认提交边界。
2. 有意暂存指定文件。
3. 运行 `python3 .agents/skills/git-ops/scripts/scan-staged-secrets.py`（无 Python 时用 PowerShell 版本），检查暂存代码是否暴露 key、secret 或 access token；只有退出码为 `0` 才准备提交信息并运行 `git commit`。
