...
- 批次证据复用前，Leader 必须核对 `base_ref`、任务清单、受影响路径/当前 diff、验证命令和环境前提组成的证据指纹；任一项变化即失效，最终 Verify 不得由复用结果替代。
...
完整命令和 `git diff <base_ref> --stat` 仍须执行；成功回传只报告统计行和可追溯记录位置，不转发重复成功日志、框架启动日志或普通警告。

## 取消与 worktree 协议

用户缩小范围、暂停或取消时，Leader 必须停止新派发、中断相关子 Agent，并等待其写入/测试进程退出和文件锁释放。只能撤销取消任务独占的未验收改动；并发测试产生的报告必须丢弃或隔离，之后的验证从干净报告目录重新开始。

创建或首次复用 worktree 时，先执行 `git -c safe.directory=<absolute-worktree> status --short`。该 worktree 内后续 Git 检查或索引写操作沿用相同参数；不得为此修改用户全局 Git 配置。
