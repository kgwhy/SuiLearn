...
批次验收证据可以复用，但必须先比对证据指纹：`base_ref`、任务清单、受影响路径/当前 diff、验证命令与其环境前提。上述任一项变化即使旧证据失效；全部一致时可以复用已完成的独立 Test/Review 结论，不重复运行全量命令。最终 Verify 仍必须执行 change 规定的最终验证，不能由复用证据替代。
...
用户缩小范围、暂停或取消时，Leader 必须立即停止新的派发并中断相关子 Agent；等待其写入和测试进程退出，确认文件锁释放后，才可处理取消任务产生的专属改动。取消后的验证报告不得与仍在运行的命令混用；需要重新验证时先清理或隔离旧测试报告，再按受影响范围运行新命令。

完整命令仍须执行并记录，但对话和子 Agent 成功回传只能给出紧凑摘要。`git diff <base_ref> --stat` 的完整结果写入验证记录或可追溯日志位置，对话只显示最终统计行；无关的成功日志、重复警告和大段框架启动输出不得转发。
...
- 创建或首次复用 worktree 时，使用绝对路径执行 `git -c safe.directory=<absolute-worktree> status --short`；此后该 worktree 内的 Git 检查和索引写操作必须携带同一 `safe.directory` 参数，不修改用户全局 Git 配置。
...
- worktree 初始化或首次访问必须通过 `git -c safe.directory=<absolute-worktree>` 进行只读状态检查；出现所有权告警时，不得以裸 `git` 重试或写入全局 `safe.directory`。
- 契约变更先于消费端适配。
- 只有契约稳定后，消费端适配才能并行开始。
